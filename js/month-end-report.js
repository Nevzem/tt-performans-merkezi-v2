...
async function exportMonthEndPNG(){var el=document.getElementById('month-end-report');if(!el)return;try{await ensureH2C();var cv=await html2canvas(el,{scale:1,backgroundColor:'#061528',useCORS:true,logging:false,width:1536,height:1024});_openSharePreview(cv.toDataURL('image/png'),'TT_AySonu_'+merDealerCode+'_'+String(merPeriod()).replace(/[^0-9A-Za-zÇĞİÖŞÜçğıöşü]/g,'')+'.png')}catch(e){alert('Görsel oluşturma hatası: '+e.message)}}
