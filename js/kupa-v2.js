...
var KB_LOW_UP_ICON = '<svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 17 9 11 13 15 21 7"/><polyline points="15 7 21 7 21 13"/></svg>';
...
/* ─── GÜNÜN EN HIZLI / EN AZ YÜKSELENİ ──────────────────────────────────
   TÜM KUPA listesi taranır (yalnızca ilk 10 değil) — en büyük pozitif ve
   en küçük pozitif günlük puan değişimine sahip bayi bulunur. Önceki
   anlık görüntü yoksa boş durum gösterilir (veri uydurulmaz). ────────── */
function _kbBiggestMovers(K, prevSnap) {
  if (!prevSnap) return { up: null, leastUp: null };
  var up = null, leastUp = null;
  K.forEach(function(row, i) {
    ...
    if (delta > 0 && (!leastUp || delta < leastUp.delta)) leastUp = { row: row, delta: delta, rankChange: rankChange };
  });
...
function renderKupaMovers(K, prevSnap) {
  ...
  var leastUpHTML = m.leastUp
    ? '<div class="kb-mini-nm">' + m.leastUp.row.b + '</div><div class="kb-mini-il">' + m.leastUp.row.kod + ' · ' + m.leastUp.row.il + '</div>' +
      '<div class="kb-mini-val lowup">+' + _kbN1(m.leastUp.delta) + '<small>PUAN</small></div>' +
